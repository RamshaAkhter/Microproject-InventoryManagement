...
app.get('/', (req,res)=>{
   db.collection('Women Clothing').find().toArray( (err,result)=>{
      if(err) return console.log(err)
...
app.post('/adddata', (req,res)=>{
   db.collection('Women Clothing').save(req.body, (err, result)=>{
      if(err) return console.log(err)
   res.redirect('/')
})
})
...
app.post('/update', (req,res)=>{
   ...
   db.collection('Women Clothing').find().toArray( (err,result)=>{
      ...
      db.collection('Women Clothing').findOneAndUpdate({pid: req.body.pid},{
...
app.post('/delete', (req,res)=>{
   db.collection('Women Clothing').findOneAndDelete({pid: req.body.pid},(err,result)=>{
        if(err) return console.log(err)
        res.redirect('/')
   })
})
